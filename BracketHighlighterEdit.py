...
class BH(object):
    ...
    def __init__(self):
        ...
        self.settings_dict['indic_for_box_at_caret'] = 1
        assert self.settings_dict['indic_for_box_at_caret'] is not None

        for editorX in (editor1, editor2):
            self.indicatorOptionsSet(self.settings_dict['indic_for_box_at_caret'], INDICATORSTYLE.STRAIGHTBOX, (0,0,0), 255, 0, True, editorX)  # white box rimmed in "pale violet red 2"
    ...
    def containing_box_indices_into_string(self, str_containing_caret, caret_index_into_str):
        ...
        for j in range(caret_index_into_str, len(str_containing_caret)):
            ...
            if (j > 0) and ((c == '<' and (str_containing_caret[j-1] == '=' or str_containing_caret[j-1] == '-')) or (c == '>' and (str_containing_caret[j-1] == '=' or str_containing_caret[j-1] == '-'))):
                continue
            if c in closing_chars: # and c_prev != '=' and c_prev != '-':
                if stack.isEmpty():
                    box_ending_index = j
                    break
                else:
                ...
                if c in opening_chars:
                    if stack.isEmpty():
                        ...
                        if stack.peek() ==  get_closing_char_via_opening_char_dict[c]:
                            ...
                            break  # unbalanced
                elif c in closing_chars: # and c_prev != '=' and c_prev != '-':
                    stack.push(c)
                c_prev = c
