...
try:
    ...
    BH__dict['indic_for_box_at_caret'] = 1  # pick a free indicator number
    ...
    for editorX in (editor1, editor2):
        indicatorOptionsSet(BH__dict['indic_for_box_at_caret'], INDICATORSTYLE.STRAIGHTBOX, (0,0,0), 255, 0, True, editorX)  # white box rimmed in "pale violet red 2"
    ...
    def BH__containing_box_indices_into_string(str_containing_caret, caret_index_into_str):
        ...
        for j in range(caret_index_into_str, len(str_containing_caret)):
            ...
            if c in closing_chars and c_prev != '=':
                if stack.isEmpty():
                    box_ending_index = j
                    break
                else:
                ...
                if c in opening_chars:
                    if stack.isEmpty():
                        ...
                        if stack.peek() ==  get_closing_char_via_opening_char_dict[c]:
                            ...
                            break  # unbalanced
                elif c in closing_chars and c_prev != '=':
                    stack.push(c)
                c_prev = c
